...
export default function SettingsPage() {
  ...
  return <>
    ...
    <section className="settings-page-grid">
      ...
      <div className="surface runtime-info"><div className="section-head"><h2>运行环境</h2><ShieldCheck size={18} /></div><dl><div><dt>币安环境</dt><dd>{data.environment === 'testnet' ? '测试网' : '实盘'}</dd></div><div><dt>系统模式</dt><dd>{data.mode}</dd></div><div><dt>暂停原因</dt><dd>{data.halt_reason ?? '无'}</dd></div><div><dt>健康门禁</dt><dd className={data.health.ready ? 'positive' : 'warning'}>{data.health.ready ? '通过' : '未通过'}</dd></div><div><dt>运行时间</dt><dd>{formatUptime(data.uptime_seconds)}</dd></div><div><dt>数据时区</dt><dd>UTC</dd></div><div><dt>界面时区</dt><dd>Asia/Shanghai</dd></div></dl><Button variant="secondary" icon={<LogOut size={15} />} onClick={() => logout.mutate()} disabled={logout.isPending}>退出登录</Button></div>
    </section>
